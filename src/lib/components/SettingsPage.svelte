...
<div class="p-6 max-w-5xl mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <h2 class="hud-strong text-2xl mt-1" style="color: var(--fg-0)">SETTINGS</h2>
    </div>
    <button class="btn-base" onclick={onBack}>← INDIETRO</button>
  </div>
  ...
  <div class="grid grid-cols-12 gap-4">
    ...
    <div class="col-span-12 flex items-center gap-3">
      <button class="btn-base btn-primary px-6 py-3" disabled={saving} onclick={save}>
    ...
    <section class="panel p-4 col-span-6">
      ...
      {#if authed}
        <div class="flex items-center gap-3">
          ...
          <span class="hud-strong" style="color: var(--accent-start)">LOGIN ATTIVO</span>
        </div>
        <button class="btn-base mt-4" onclick={doLogout}>LOGOUT</button>
      {:else}
        <div class="flex items-center gap-3">
          <span class="dot-idle" style="background: var(--accent-finish)"></span>
          <span class="hud-strong" style="color: var(--accent-finish)">NON AUTENTICATO</span>
        </div>
        <button class="btn-base btn-primary mt-4" onclick={() => showLogin = true}>ACCEDI</button>
      {/if}
    </section>
    ...
    <section class="panel p-4 col-span-6">
      ...
      <button class="btn-base btn-primary" onclick={doSyncData}>SINCRONIZZA ATLETI/PERCORSI</button>
      {#if syncStatus}
        <div class="hud mt-3" style="color: var(--fg-1)">{syncStatus}</div>
      {/if}
    </section>
    ...
    <section class="panel p-4 col-span-12">
      ...
      <button class="btn-base" onclick={doExport}>ESPORTA RISULTATI XLSX</button>
    </section>
  </div>
</div>
