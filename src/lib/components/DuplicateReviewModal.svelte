  ...
  <div class="panel-2 w-full max-w-4xl mx-auto">
    <div class="px-5 py-3 flex items-center justify-between border-b" style="border-color: var(--line-2)">
      <div class="flex items-center gap-3">
        ...
        <div class="hud-strong" style="color: var(--accent-dup)">{$t.modals.duplicateReview.title}</div>
        <div class="hud">·</div>
        <div class="hud">{groups.length}</div>
      </div>
      <div class="flex gap-2">
        <Button variant="ghost" size="sm" class="px-2 py-1" onclick={refresh}>
          {$t.modals.duplicateReview.refreshButton}
        </Button>
    ...
    <div class="p-4">
      {#if loading}
        <div class="hud" style="color: var(--fg-2)">{$t.common.loading}</div>
      {:else if groups.length === 0}
        <div class="hud" style="color: var(--accent-start)">
          <Check size={14} /> {$t.modals.duplicateReview.noDuplicates}
        </div>
      {:else}
              ...
              <div class="flex items-center justify-between px-4 py-2 border-b" style="border-color: var(--line-2)">
                <div class="flex items-center gap-3">
                  <span class="hud-strong" style="color: var(--accent-dup)">
                    {$t.modals.duplicateReview.bibLabel} #{g.bib_number ?? '?'}
                  </span>
                  <span class="hud">Δ</span>
                  <span class="num text-lg" style="color: var(--fg-0)">{g.delta_ms}ms</span>
                </div>
                <span class="hud" style="color: var(--fg-3)">
                  {g.timings.length} {$t.modals.duplicateReview.readingsLabel}
                </span>
              </div>
              <div>
                {#each g.timings as t_ (t_.id)}
                  <div class="ticker-row">
                    <span class="op-chip" style="color: var(--fg-2)">{t_.operator_id}</span>
                    <span class="num" style="color: var(--fg-0)">
                      {formatMsToHms(t_.total_time_ms ?? 0)}
                    </span>
                    <span class="hud ml-auto" style="color: var(--fg-3)">id #{t_.id}</span>
                  </div>
                {/each}
              </div>
            </div>
